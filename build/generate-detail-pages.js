...
/* Ancorele externe si blocul Explore se iau din pagina canonica, ca sa nu
   existe doua versiuni ale aceleiasi liste. */
const canonicalPage = readFileSync(join(ROOT, 'pricing.html'), 'utf8');

let SAME_AS = [];
try {
  const m = canonicalPage.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/);
  const g = JSON.parse(m[1])['@graph'] || [];
  const org = g.find(o => String(o['@id'] || '').endsWith('#organization'));
  if (org && Array.isArray(org.sameAs)) SAME_AS = org.sameAs;
} catch {}

/* Blocul Explore sta imediat inaintea footerului. Il luam intreg, cu
   navigatia si legaturile catre restul retelei. */
let EXPLORE = '';
try {
  const fi = canonicalPage.lastIndexOf('<footer');
  const hub = canonicalPage.lastIndexOf('<nav', fi);
  if (hub > 0 && fi > hub) EXPLORE = canonicalPage.slice(hub, fi);
} catch {}
  ...
  const graph = {
    ...
    '@graph': [
      ...
      { '@type': ['Organization', 'Corporation'], '@id': SITE + '/#organization',
        ...
        /* sameAs preluat din graful canonic, nu rescris aici: ancorele externe
           sunt semnalul cel mai puternic impotriva confuziei de entitate, si
           o lista scurtata pe cateva pagini ar slabi exact ce intareste. */
        sameAs: SAME_AS },
...
/* Sablonul se taie la </style>, deci </head> ramane afara. Il punem inapoi
   explicit, impreuna cu graful — altfel inserarea inainte de </head> nu
   gaseste nimic si paginile ies fara JSON-LD, adica fara sa declare ce sunt.
   Exact ce s-a intamplat la prima generare. */
const styleEnd = template.indexOf('</style>') + '</style>'.length;
const shell = template.slice(0, styleEnd);

/* Subsolul se ia dintr-o pagina cu subsol complet — Explore, contact, reteaua
   AiVenture si toate legaturile sociale. signal-detail.html are unul redus,
   iar paginile generate din el mosteneau lipsa. */
const footSource = readFileSync(join(ROOT, 'pricing.html'), 'utf8');
const footStart = footSource.lastIndexOf('<footer');
const foot = footStart > 0 ? footSource.slice(footStart) : '</body></html>';
