...
async function handleRequest(request, env, ctx, requestId) {
  {
    ...
    if (url.pathname === '/audit' && request.method === 'POST') {
      ...
      /* ---------- observatia anterioara pentru acelasi site ----------
         Prima varianta lista pana la 400 de chei si citea FIECARE, secvential,
         ca sa gaseasca ultima observatie a aceluiasi URL. 400 de citiri KV in
         serie inseamna zeci de secunde — am reparat un timeout de 10 secunde
         si am introdus unul de 60.

         Acum tinem un pointer: o singura cheie per URL, care retine ultima
         observatie. O citire in loc de patru sute, si scrierea pointerului se
         face dupa ce raspunsul a plecat, deci nu costa nimic din timpul
         clientului. */
      if (env.RATE_KV) {
        const urlKey = 'obslast:' + report.url;
        try {
          const pointer = await env.RATE_KV.get(urlKey);
          if (pointer) {
            const raw = await env.RATE_KV.get('obs:' + pointer);
            if (raw) {
              const best = JSON.parse(raw);
              const at = best.at || best.observedAt || best.ts;
              if (at && at < report.observedAt) {
        ...
        /* Pointerul se actualizeaza in fundal. ctx.waitUntil tine invocarea in
           viata pentru scriere, dar raspunsul a fost deja trimis. */
        try {
          const write = env.RATE_KV.put(urlKey, report.observationId,
      ...
      /* Stocarea se face in fundal. Clientul nu are ce astepta de la ea:
         raportul e deja complet in raspuns, iar contorul si arhiva sunt
         pentru mai tarziu. Trei operatii KV asteptate sincron adaugau ~300ms
         la fiecare audit, degeaba. */
      if (env.RATE_KV) {
        const persist = (async () => {
          try {
            const n = Number(await env.RATE_KV.get('audit_count') || 0) + 1;
            await env.RATE_KV.put('audit_count', String(n));
            await env.RATE_KV.put('obs:' + report.observationId, JSON.stringify(report),
              { expirationTtl: 60 * 60 * 24 * 90 });
          } catch {}
        })();
        if (ctx && ctx.waitUntil) ctx.waitUntil(persist); else await persist;
      }
      return json(report);
    }
    ...
    if (url.pathname === '/stats') {
      ...
      return json({ audits: count, version: '3.6.0', engine: 'evidence-based', signals: 167, external_sources: ['PageSpeed Insights (free)'], brand: '3webs', network: '5thElement.ai', a2a: '/a2a', agent_card: '/.well-known/agent-card.json' });
    }
