...
function observationExpirationUnix(observedAt = new Date()) {
  const d = observedAt instanceof Date ? new Date(observedAt.getTime()) : new Date(observedAt);
  if (Number.isNaN(d.getTime())) throw new Error('Invalid observation date');
...
function buildActionPlan(report) {
  ...
  return {
    ...
    optimize: { title: 'Refine', items: chunk(top, 2).map(s => s.n), impact: 'Fine adjustments', delta: 'points within 3-years' },
  };
}
...
async function handleRequest(request, env, ctx, requestId) {
  ...
  if (url.pathname === '/faq' || url.pathname === '/faq/') {
    ...
    return env.ASSETS.fetch(new Request(faqUrl, request));
  }
if (request.method === 'OPTIONS') return new Response(null, { headers: CORS });
    ...
    if (url.pathname === '/audit' && request.method === 'POST') {
      ...
      if (env.RATE_KV) {
        try {
          ...
          await env.RATE_KV.put('obs:' + report.observationId, JSON.stringify(report), { expiration: observationExpirationUnix(new Date(report.observedAt || report.timestamp || report.ts || Date.now())) });
        } catch {}
      }
      return json(report);
    }
    ...
    if (url.pathname === '/lead' && request.method === 'POST') {
      ...
      return json({
        ...
        note: record.delivered
          ? 'Sent. Check your inbox — the report link stays live for 3-years.'
    ...
    if (url.pathname === '/observation' && request.method === 'GET') {
      ...
      if (!stored) return json({ error: 'no stored observation with id ' + id,
        detail: 'Observations are retained for 3-years.' }, 404);
      ...
      if (skill === 'obs_one_shot') {
        ...
        if (env.RATE_KV) {
          try {
            ...
            await env.RATE_KV.put('obs:' + observationId, JSON.stringify(report), { expiration: observationExpirationUnix(new Date(report.observedAt || report.timestamp || report.ts || Date.now())) });
          } catch {}
        }
